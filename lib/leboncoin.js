...
request(url, function (error, response, body) { //make the request to the web page
  if (!error) {
    ...
    json.properties.price = parseInt($("[itemprop='price']").text()); //find what we are interested in.
	...
	fs.writeFile('resultLBC.json', JSON.stringify(json, null, 4), function(err){ //write the result in a new JSON file, the schema is kept untouched
...
//scrape('http://www.leboncoin.fr/ventes_immobilieres/920844062.htm?ca=12_s');
